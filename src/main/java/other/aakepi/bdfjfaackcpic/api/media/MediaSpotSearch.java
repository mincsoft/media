 ...
 */
public class MediaSpotSearch extends BaseApiSupport implements ApiSupport {
    ...
    public String execute(Request request, Long userId, Long tenantId) {
        ...
        JSONObject sheetObj = sheet(request, startAt, endAt, null);
     ...
     * @param request
     */
    private void initParam(Request request) {
     ...
     * @return
     */
    private JSONObject sheet(Request request, String startAt, String endAt, String sheetStr) {
...
//        columns.add(getCol("Owner","所有者","80",null));
        ...
        Date startDate = DateUtil.getDate(startAt);
        Date endDate = DateUtil.getDate(endAt);
        ...
        try {
            for (int i = 0; i < columns.size(); i++) {
                ...
                //全部字段后显示点位：
                boolean renderDateColumn = lastSpotColumn == columns.size();//false;
                if (renderDateColumn) {
...
//								groupJson.accumulate("end", (lastSpotColumn + dateColumns));
        ...
        //加载媒体的数据：
        JSONArray cellArray = getMediaJson(request, 0, 300, sheetId);
...
//    log.info(floatingArray);
     ...
     * @return
     */
    private JSONArray getMediaJson(Request request, int first, int size, int sheetId) {
        JSONArray headData = new JSONArray();
        QueryResult result = getAllMedia(request, first, size);
...
//                headData.add(getColItemObject(sheetId, startRow, dateColumns++, record.get("name")));
//                headData.add(getColItemSelect(sheetId, startRow, dateColumns++, record, "entityType"));
//                headData.add(getColItemSelect(sheetId, startRow, dateColumns++, record, "opMode"));
//                headData.add(getColItemSelect(sheetId, startRow, dateColumns++, record, "saleStatus"));
//                headData.add(getColItemObject(sheetId, startRow, dateColumns++, record.get("address")));
//                headData.add(getColItemObject(sheetId, startRow, dateColumns++, record.get("retailPrice")));
//                headData.add(getColItemObject(sheetId, startRow, dateColumns++, record.get("ownerId")));
